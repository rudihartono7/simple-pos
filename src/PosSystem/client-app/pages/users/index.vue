  ...
  <div class="p-3 sm:p-4 lg:p-6">
    ...
    <div class="mb-4 sm:mb-6 lg:mb-8">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between space-y-3 sm:space-y-0">
        <div>
          <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-black">User Management</h1>
          <p class="text-neutral-gray mt-1 sm:mt-2 text-sm sm:text-base">Manage system users and their permissions</p>
        </div>
        <button
          @click="openCreateModal"
          class="bg-gradient-to-r from-primary to-primary-dark hover:from-primary-dark hover:to-primary text-black px-4 sm:px-6 py-2 sm:py-3 rounded-xl font-medium shadow-lg hover:shadow-xl transition-all duration-200 flex items-center space-x-2 text-sm sm:text-base w-full sm:w-auto justify-center"
        >
    ...
    <!-- Search and Filters -->
    <div class="bg-white rounded-2xl shadow-sm border border-neutral-medium p-3 sm:p-4 lg:p-6 mb-4 sm:mb-6">
      <div class="flex flex-col sm:flex-row gap-3 sm:gap-4 items-stretch sm:items-center">
        <div class="flex-1 min-w-0">
          <div class="relative">
            <svg class="absolute left-2.5 sm:left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 sm:w-5 sm:h-5 text-neutral-gray" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <input
              ...
              class="w-full pl-8 sm:pl-10 pr-3 sm:pr-4 py-2.5 sm:py-3 border border-neutral-medium rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent transition-all duration-200 text-sm sm:text-base"
            />
          </div>
        </div>
        <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
          <select
            v-model="roleFilter"
            class="px-3 sm:px-4 py-2.5 sm:py-3 border border-neutral-medium rounded-xl focus:ring-2 focus:ring-primary focus:border-transparent bg-white text-sm sm:text-base"
          >
          ...
          <button
            @click="loadUsers"
            :disabled="loading"
            class="bg-neutral-light hover:bg-neutral-medium text-neutral-gray px-3 sm:px-4 py-2.5 sm:py-3 rounded-xl font-medium disabled:opacity-50 transition-all duration-200 flex items-center justify-center space-x-2 text-sm sm:text-base"
          >
    ...
    <div v-if="loading" class="flex items-center justify-center py-8 sm:py-12">
      <div class="text-center">
        ...
        <p class="mt-3 sm:mt-4 text-neutral-gray font-medium text-sm sm:text-base">Loading users...</p>
      </div>
    </div>
    ...
    <div v-else-if="filteredUsers.length > 0" class="grid gap-3 sm:gap-4">
      <div
        ...
        :key="userItem.id"
        class="bg-white rounded-2xl shadow-sm border border-neutral-medium p-3 sm:p-4 lg:p-6 hover:shadow-md transition-all duration-200"
      >
        <div class="flex flex-col sm:flex-row sm:items-center justify-between space-y-3 sm:space-y-0">
          <div class="flex items-center space-x-3 sm:space-x-4 flex-1">
            <div class="flex-shrink-0">
              <div class="h-10 w-10 sm:h-12 sm:w-12 rounded-full bg-gradient-to-br from-primary to-primary-dark flex items-center justify-center shadow-lg">
                <span class="text-sm sm:text-lg font-bold text-black">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-3 mb-1">
                ...
                <div class="flex flex-wrap gap-2">
                  ...
                  <span class="px-2 sm:px-3 py-1 text-xs font-medium rounded-full bg-primary-light text-primary border border-primary w-fit">
                    {{ userItem.role }}
                  </span>
                </div>
              </div>
              <div class="text-xs sm:text-sm text-neutral-gray space-y-1">
                <div class="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-4">
                  <span class="flex items-center space-x-1">
                    <svg class="w-3 h-3 sm:w-4 sm:h-4 text-neutral-gray" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex items-center space-x-2 mt-3 sm:mt-0">
            <button
              ...
              class="bg-primary-light hover:bg-primary text-primary hover:text-black px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-medium transition-all duration-200 flex items-center space-x-1"
            >
              <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="relative">
              <button
                ...
                class="bg-neutral-light hover:bg-neutral-medium text-neutral-gray px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-medium transition-all duration-200"
              >
                <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div
                ...
                class="absolute right-0 mt-2 w-44 sm:w-48 bg-white rounded-xl shadow-lg border border-neutral-medium py-2 z-10"
              >
                <button
                  v-if="userItem.isActive"
                  @click="deactivateUser(userItem)"
                  class="w-full text-left px-3 sm:px-4 py-2 text-xs sm:text-sm text-warning hover:bg-warning-light flex items-center space-x-2"
                >
                  <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  class="w-full text-left px-3 sm:px-4 py-2 text-xs sm:text-sm text-success hover:bg-success-light flex items-center space-x-2"
                >
                  <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  class="w-full text-left px-3 sm:px-4 py-2 text-xs sm:text-sm text-primary hover:bg-primary-light flex items-center space-x-2"
                >
                  <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  class="w-full text-left px-3 sm:px-4 py-2 text-xs sm:text-sm text-danger hover:bg-danger-light flex items-center space-x-2"
                >
                  <svg class="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-else class="text-center py-8 sm:py-12 lg:py-16">
      <div class="mx-auto w-16 h-16 sm:w-20 sm:h-20 lg:w-24 lg:h-24 bg-neutral-light rounded-full flex items-center justify-center mb-4 sm:mb-6">
        <svg class="w-8 h-8 sm:w-10 sm:h-10 lg:w-12 lg:h-12 text-neutral-gray" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <h3 class="text-lg sm:text-xl font-semibold text-black mb-2">No users found</h3>
      <p class="text-neutral-gray mb-4 sm:mb-6 text-sm sm:text-base">Get started by creating your first user account.</p>
      <button
        @click="openCreateModal"
        class="bg-gradient-to-r from-primary to-primary-dark hover:from-primary-dark hover:to-primary text-black px-4 sm:px-6 py-2 sm:py-3 rounded-xl font-medium shadow-lg hover:shadow-xl transition-all duration-200 inline-flex items-center space-x-2 text-sm sm:text-base"
      >
        <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
        </svg>
    ...
    <div v-if="showModal" class="fixed inset-0 modal-backdrop overflow-y-auto h-full w-full z-50">
      <div class="relative top-4 sm:top-10 mx-auto p-3 sm:p-5 border w-full max-w-lg shadow-lg rounded-md bg-white">
        <div class="mt-3">
          <h3 class="text-base sm:text-lg font-medium text-black mb-4">
          ...
          <form @submit.prevent="saveUser" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  type="text"
                  required
                  class="w-full px-3 py-2 border border-neutral-medium rounded-md focus:ring-primary focus:border-primary text-sm sm:text-base"
            ...
            <div>
              <label class="block text-sm font-medium text-black mb-1">Store</label>
              <select
            ...
            <div>
              <label class="block text-sm font-medium text-black mb-1">PIN (4 digits)</label>
              <input
            ...
            <div v-if="isEditing">
              <label class="flex items-center">
                <input
                  ...
                  type="checkbox"
                  class="rounded border-neutral-medium text-primary shadow-sm focus:border-primary focus:ring focus:ring-primary focus:ring-opacity-50"
                />
            ...
            <div class="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-3 pt-4">
              ...
              <button
                type="submit"
                :disabled="saving"
                class="px-4 py-2 text-sm font-medium text-black bg-gradient-to-r from-primary to-primary-dark hover:from-primary-dark hover:to-primary rounded-md disabled:opacity-50"
              >
    ...
    <div v-if="showResetPasswordModal" class="fixed inset-0 modal-backdrop overflow-y-auto h-full w-full z-50">
      <div class="relative top-10 sm:top-20 mx-auto p-3 sm:p-5 border w-full max-w-sm sm:w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
          <h3 class="text-lg font-medium text-black mb-4">Reset Password</h3>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 modal-backdrop overflow-y-auto h-full w-full z-50">
      <div class="relative top-10 sm:top-20 mx-auto p-3 sm:p-5 border w-full max-w-sm sm:w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
          <div class="mx-auto flex items-center justify-center h-12 w-12 rounded-full bg-danger-light">
            <svg class="h-6 w-6 text-danger" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"></path>
            </svg>
          </div>
          <h3 class="text-lg font-medium text-black mt-2">Delete User</h3>
          <p class="text-sm text-neutral-gray mt-2">
          ...
          <div class="flex justify-center space-x-3 mt-4">
            ...
            <button
              @click="deleteUser"
              :disabled="deleting"
              class="px-4 py-2 text-sm font-medium text-white bg-danger hover:bg-danger-dark rounded-md disabled:opacity-50"
            >
